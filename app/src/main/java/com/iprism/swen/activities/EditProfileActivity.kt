...
import com.iprism.swen.databinding.ActivityEditProfileBinding
import com.iprism.swen.models.addfamilymember.AddFamilyMemberRequest
import com.iprism.swen.models.profile.ProfileItem
...
import com.iprism.swen.models.userdropdowns.Response
...
import com.iprism.swen.viewmodels.AddFamilyMemberViewModel
...
class EditProfileActivity : AppCompatActivity() {
    ...
    private var currentProfile: ProfileItem? = null
    ...
    private fun handleFamilyBtn() {
        binding.addFamilyBtn.setOnClickListener(View.OnClickListener {
            if (profileUri == null) {
                ...
                val request = AddFamilyMemberRequest(convertUriToBase64Image(profileUri)!!, genderId!!, coverageCategoriesId!!.toInt(), userDetails[User.ID]!!.toInt(), getDob(), getName(), getMobile(), bloodGroupId!!.toInt(), getEmail(), "")
                val intent = Intent(this, SingleSubscriptionActivity::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK && data != null) {
            ...
            val uri: Uri = getImageUri(this@EditProfileActivity, bitmap)!!
        ...
        viewModel.profileResponse.observe(this) { result ->
            when (result) {
                ...
                is UiState.Success -> {
                    ...
                    binding.subscriptionIdTxt.text = "Subscription Id : " + result.data.response.subscriptionId
    ...
    private fun observeProfileEditResponse() {
        viewModel.profileEditResponse.observe(this) { result ->
            when (result) {
                is UiState.Loading -> {
                    binding.progress.showProgress()
                }
    ...
    private fun showDetails(details: ProfileItem?) {
        if (details == null) {
           /* binding.nameTxt.setText("")
            ...
            binding.headingNameTxt.visibility = View.GONE
            binding.dobTxt.text = ""
            profileUri = null
            ...
            binding.mobileEt.isClickable = true
            binding.mobileEt.isFocusable = true
            binding.mobileEt.isFocusableInTouchMode = true

            binding.emailTxt.isClickable = true
            binding.emailTxt.isFocusable = true
            binding.emailTxt.isFocusableInTouchMode = true*/
        } else {
            currentProfile = details
            binding.nameTxt.setText(details.name)
            binding.relationshipEt.setText(details.relationship)
            binding.headingNameTxt.setText(details.name)
            ...
            setupBloodGroupsSpinner(dropDowns!!.bloodGroups)
            setupCoverageCategoriesSpinner(dropDowns!!.categories)
            binding.mobileEt.isClickable = false
            binding.mobileEt.isFocusable = false
            binding.emailTxt.isClickable = false
            binding.emailTxt.isFocusable = false
        }
    }
    ...
    private fun profileEdit() {
        val userDetails = getUserDetails()
        val request = ProfileEditRequest(convertUriToBase64Image(profileUri)!!, genderId!!, userDetails[User.ID]!!.toInt(), getDob(), getName(), getMobile(), bloodGroupId!!.toInt(), userDetails[User.AUTH_TOKEN]!!.toString(), currentProfile!!.type, getEmail(), currentProfile!!.id)
        NetworkRetryHelper.checkAndCallWithRetry(this, request) { req ->
            viewModel.profileEdit(req)
        }
        Log.d("requestLoading", request.toString())
    }
